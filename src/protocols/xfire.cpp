...
std::list<std::string> XFireProtocol::transportFeatures(){
	return m_transportFeatures;
}

std::list<std::string> XFireProtocol::buddyFeatures(){
	return m_buddyFeatures;
}

std::string XFireProtocol::text(const std::string &key) {
	if (key == "instructions")
...
Tag *XFireProtocol::getVCardTag(AbstractUser *user, GList *vcardEntries) {
	Tag *N = new Tag("N");
...
void XFireProtocol::onPurpleAccountCreated(PurpleAccount *account) {
//	purple_account_set_string(account, "server", "login.xfire.com");
// 	purple_account_set_bool(account, "use_clientlogin", 0);
// 	purple_account_set_bool(account, "use_ssl", 0);
 	purple_account_set_int(account, "version", 127);
}

bool XFireProtocol::onPurpleRequestInput(void *handle, AbstractUser *user, const char *title, const char *primary,const char *secondary, const char *default_value,gboolean multiline, gboolean masked, gchar *hint,const char *ok_text, GCallback ok_cb,const char *cancel_text, GCallback cancel_cb, PurpleAccount *account, const char *who,PurpleConversation *conv, void *user_data) {
	if (primary){
...
SPECTRUM_PROTOCOL(xfire, XFireProtocol)
